...
export default function ReportEditForm({
  ...
  const [isWeekly, setIsWeekly] = useState(report.isWeekly);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/reports/${report.id}`, {
        ...
        body: JSON.stringify({ title: title.trim(), body: body.trim(), isWeekly: canPublishWeekly ? isWeekly : report.isWeekly }),
      });
    ...
    <form onSubmit={handleSubmit} className="space-y-4 rounded border border-[var(--border)] bg-[var(--bg-card)] p-6 shadow-sm">
      ...
      {canPublishWeekly && (
        <label className="flex items-center gap-2">
          ...
          />
          <span className="text-sm text-[var(--text-secondary)]">作为本周研报</span>
        </label>
      )}
